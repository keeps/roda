...
public class DefaultSearchParameter extends SearchParameter {
  ...
  public DefaultSearchParameter(DefaultSearchParameter searchParameter) {
    this(searchParameter.getFields(), searchParameter.getValue(), searchParameter.getConstraint());
  }

  /**
   * Constructs a new {@link DefaultSearchParameter} with the given parameters.
   * 
   * @param fields
   ...
   *           </ul>
   */
  public DefaultSearchParameter(String[] fields, String value, int constraint) throws IllegalArgumentException {
    setFields(fields);
    setValue(value);
  ...
  public boolean equals(Object obj) {
    if (obj != null && obj instanceof DefaultSearchParameter) {
      ...
      return this.getConstraint() == other.getConstraint() && this.fields.equals(other.fields)
  ...
  public String toString() {
    return "DefaultSearchParameter (" + constraintToString(getConstraint()) + "," + Arrays.asList(getFields()) + ", "
      + getValue() + ")";
  }
  ...
  public void setConstraint(int constraint) throws IllegalArgumentException {
    if (constraint == MATCH_EXACT_PHRASE || constraint == MATCH_AT_LEAST_ONE_WORD || constraint == MATCH_WITHOUT_WORDS
    ...
    switch (getConstraint()) {
      case DefaultSearchParameter.MATCH_AT_LEAST_ONE_WORD:
        ...
        for (int i = 0; i < fields.size(); i++) {
          ...
          subQuery += escapedKeyword(getValue().trim().replaceAll("\\s+", " OR "));
          subQuery += ")";
          if (i + 1 < fields.size()) {
            subQuery += " OR ";
          }
      ...
      case DefaultSearchParameter.MATCH_ALL_WORDS:
        ...
        for (int i = 0; i < fields.size(); i++) {
          subQuery += ((String) fields.get(i)) + ":(";
          subQuery += escapedKeyword(getValue().trim().replaceAll("\\s+", " AND "));
          subQuery += ")";
          if (i + 1 < fields.size()) {
            subQuery += " OR ";
          }
