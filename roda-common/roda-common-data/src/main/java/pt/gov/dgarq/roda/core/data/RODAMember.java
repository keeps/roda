...
public class RODAMember implements Serializable {
	...
	public RODAMember(RODAMember member) {
		this(member.isActive(), member.getName(), member.getFullName(), member
				.getGroups(), member.getAllGroups(), member.getDirectRoles(),
				member.getRoles());
	}
	 ...
	 */
	public RODAMember(boolean active, String name, String fullName,
			String[] groups, String[] allGroups, String[] directRoles,
			String[] roles) {
	...
	public String toString() {
		return "RODAMember (active=" + (isActive() ? "active" : "inactive")
				+ ", name=" + getName() + ", fullName=" + getFullName()
				+ ", groups=" + Arrays.toString(getGroups()) + ", allGroups="
				+ Arrays.toString(getAllGroups()) + ", directRoles="
				+ Arrays.toString(getDirectRoles()) + ", roles="
				+ Arrays.toString(getRoles()) + ")";
	}
	...
	public String[] getAllGroups() {
		return (String[]) this.allGroups.toArray(new String[this.allGroups
				.size()]);
	}
	 ...
	 *            the name to test.
	 * @return
	 *         <code>true<code> if the name is valid,<code>false<code> otherwise.
	 */
	public boolean isNameValid(String name) {
		boolean isValid = true;
		for (int i = 0; i < name.length(); i++) {
			char currentChar = name.charAt(i);
			isValid &= Character.isLetterOrDigit(currentChar)
