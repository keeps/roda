...
public class DescriptionLevelManager implements Serializable {
	 ...
	 * specified parentLevel
	 * */
	public static List<DescriptionLevel> getChildLevels(
			DescriptionLevel parentLevel) {
	 ...
	 * specified childLevel
	 * */
	public static List<DescriptionLevel> getParentLevels(
			DescriptionLevel childLevel) {
	...
	/**
	 * Non-cloned version of the list containing representation description
	 * levels
	 * */
	...
	private Set<String> getAllDescendants(String level) {
		...
		for (DescriptionLevel descendant : PARENT_TO_CHILDREN_MAPPING
				.get(level)) {
			descendants.add(descendant.getLevel());
			descendants.addAll(getAllDescendants(descendant.getLevel(),
					visitedLevels));
	...
	private void loadDescriptionLevelHierarchy(
			Map<Object, Object> descriptionLevels) {
		...
		for (Entry<Object, Object> entry : descriptionLevels.entrySet()) {
			...
			if (key.equals("roots")) {
				...
				for (DescriptionLevel childrenLevel : childLevels) {
					tempList = CHILDREN_TO_PARENT_MAPPING.get(childrenLevel
							.getLevel());
					if (tempList == null) {
						tempList = new ArrayList<DescriptionLevel>();
						tempList.add(new DescriptionLevel(level));
						CHILDREN_TO_PARENT_MAPPING.put(
								childrenLevel.getLevel(), tempList);
					} else {
		...
		List<String> locales = splitList((String) descriptionLevels
				.get("locales"));
		for (DescriptionLevelInfo level : DESCRIPTION_LEVELS_INFO) {
			String category = (String) descriptionLevels.get("category."
					+ level.getLevel());
			level.setCategory(DescriptionLevelCategory.valueOf(category));
			for (String locale : locales) {
				String label = (String) descriptionLevels.get("label." + locale
						+ "." + level.getLevel());
				if (label != null) {
		...
		ALL_BUT_REPRESENTATIONS_DESCRIPTION_LEVELS = new ArrayList<DescriptionLevel>(
				DESCRIPTION_LEVELS);
		ALL_BUT_REPRESENTATIONS_DESCRIPTION_LEVELS
				.removeAll(DescriptionLevelManager.REPRESENTATION_DESCRIPTION_LEVELS);
	}
