...
public final class HTMLUtils {
  ...
  private static InputStream getStylesheetInputStream(String xsltFolder, Locale locale, String filename) {
    ...
    InputStream transformerStream = classLoader.getResourceAsStream(xsltFolder + "/" + locale.getLanguage() + "/"
      + locale.getCountry() + "/" + filename + ".xslt");
    if (transformerStream == null) {
      transformerStream = classLoader.getResourceAsStream(xsltFolder + "/" + locale.getLanguage() + "/" + filename
        + ".xslt");
      if (transformerStream == null) {
  ...
  public static String preservationObjectFromStorageToHtml(SimplePreservationMetadata object, ModelService model,
    ...
    try {
      ...
      StoragePath storagePath = ModelUtils.getPreservationFilePath(object.getAipId(), object.getRepresentationId(),
        object.getFileId());
   ...
   * }
   * 
   * public static Element descriptiveMetadataFromSolrToHtml(DescriptiveMetadata
   * metadata, ModelService model) throws StorageActionException,
   * IndexActionException{ Element wrapper = new Element(Tag.valueOf("span"),
   * "").attr("type", "descriptiveMetadata"); StoragePath storagePath =
   ...
   * "fileID", "Identificador do ficheiro", rfpo.getFileId()); // TODO FIXITIES
   * wrapper = appendElement(wrapper, "formatDesignationName",
   ...
   * rfpo.getObjectCharacteristicsExtension()); wrapper = appendElement(wrapper,
   * "originalName", "Nome original", rfpo.getOriginalName()); wrapper =
   * appendElement(wrapper, "preservationLevel", "Nível de preservação",
   * rfpo.getPreservationLevel()); wrapper = appendElement(wrapper, "pronomId",
   * "ID Pronom", rfpo.getPronomId()); wrapper = appendElement(wrapper,
   * "representationId", "Identificador da representação",
   * rfpo.getRepresentationId()); wrapper = appendElement(wrapper,
   * "representationObjectId", "Identificador do objeto da representação",
   * rfpo.getRepresentationObjectId()); wrapper = appendElement(wrapper, "size",
   * "Tamanho", rfpo.getSize()); wrapper = appendElement(wrapper, "state",
   * "Estado", rfpo.getState()); wrapper = appendElement(wrapper, "type",
   * "Tipo", rfpo.getType()); return wrapper; }
   * 
   * public static Element eventPreservationObjectToHtml(EventPreservationObject
   * epo) { Element wrapper = new Element(Tag.valueOf("span"), "").attr("type",
   * "eventPreservationObject"); wrapper = appendElement(wrapper, "agentID",
   * "Identificador do agente", epo.getAgentID()); wrapper =
   * appendElement(wrapper, "agentRole", "Role do agente", epo.getAgentRole());
   * wrapper = appendElement(wrapper, "aipID", "Identificador do AIP",
   * epo.getAipId()); wrapper = appendElement(wrapper, "dateCreated",
   * "Data de criação", epo.getCreatedDate()); wrapper = appendElement(wrapper,
   * "date", "Data", epo.getDate()); wrapper = appendElement(wrapper,
   * "datetime", "Data e hora", epo.getDatetime()); wrapper =
   * appendElement(wrapper, "description", "Descrição", epo.getDescription());
   * wrapper = appendElement(wrapper, "eventDetail", "Detalhes do evento",
   * epo.getEventDetail()); wrapper = appendElement(wrapper, "eventType",
   * "Tipo de evento", epo.getEventType()); wrapper = appendElement(wrapper,
   * "fileID", "Identificador do ficheiro", epo.getFileId()); wrapper =
   ...
   * "Modelo", epo.getModel()); wrapper = appendElement(wrapper, "name", "Nome",
   * epo.getName()); wrapper = appendElement(wrapper, "objectIDs",
   ...
   * representationPreservationObjectToHtml(RepresentationPreservationObject
   * rpo) { Element wrapper = new Element(Tag.valueOf("span"), "").attr("type",
   * "representationPreservationObject"); wrapper = appendElement(wrapper,
   * "aipID", "Identificador do AIP", rpo.getAipId()); wrapper =
   * appendElement(wrapper, "dateCreated", "Data de criação",
   ...
   * rpo.getRepresentationObjectID()); wrapper = appendElement(wrapper, "state",
   * "Estado", rpo.getState()); wrapper = appendElement(wrapper, "type", "Tipo",
   * rpo.getType()); return wrapper; }
   * 
   * 
   ...
   * wrapper = appendElement(wrapper, "type", "Tipo", representation.getType());
   * wrapper = appendElement(wrapper, "dateCreated", "Data de criação",
   * representation.getDateCreated()); wrapper = appendElement(wrapper,
   * "dateModified", "Data de alteração", representation.getDateModified());
   * wrapper = appendElement(wrapper, "fileIds", "Identificadores de ficheiros",
   * representation.getFileIds()); wrapper = appendElement(wrapper, "statuses",
   * "Estados", representation.getStatuses()); return wrapper; }
   * 
   * private static Element appendElementGeneric(Element wrapper, String field,
   * String label, Object value) { if (value instanceof List) { List<String>
   * valueList = (List) value; return appendElement(wrapper, field, label,
   * valueList); } else if (value instanceof Boolean) { Boolean valueBoolean =
   * (Boolean) value; return appendElement(wrapper, field, label, valueBoolean);
   * } else if (value instanceof Date) { Date valueDate = (Date) value; return
   * appendElement(wrapper, field, label, valueDate); } else if (value
   * instanceof String) { String valueString = (String) value; return
   * appendElement(wrapper, field, label, valueString); } else if (value
   * instanceof Double) { Double valueDouble = (Double)value; return
   ...
   * } }
   * 
   * private static Element appendElement(Element wrapper, String field, String
   * label, String value) { if(value!=null){ Element el =
   ...
   * Element appendElement(Element wrapper, String field, String label, Boolean
   * value) { if(value!=null){ Element el =
   * wrapper.appendElement("span").attr("field", field);
   * el.appendElement("span").attr("type", "label").text(label);
   * el.appendElement("span").attr("type", "value").text(""+value); } return
   * wrapper; }
   * 
   * private static Element appendElement(Element wrapper, String field, String
   * label, long value) { if(value!=-1){ //TODO: any better way??? Element el =
   * wrapper.appendElement("span").attr("field", field);
   * el.appendElement("span").attr("type", "label").text(label);
   * el.appendElement("span").attr("type", "value").text(Long.toString(value));
   * } return wrapper; }
   * 
   * private static Element appendElement(Element wrapper, String field, String
   * label, Date value) { if(value!=null){ Element el =
   * wrapper.appendElement("span").attr("field", field);
   * el.appendElement("span").attr("type", "label").text(label);
   * el.appendElement("span").attr("type",
   ...
   * 
   * private static Element appendElement(Element wrapper, String field, String
   * label, List<String> values) { if(values!=null && values.size()>0){ Element
   * el = wrapper.appendElement("span").attr("field", field);
   * el.appendElement("span").attr("type", "label").text(label); Element
   * valuesWrapper = el.appendElement("span").attr("type", "values"); for
   * (String string : values) { valuesWrapper.appendElement("span").attr("type",
   * "value").text(string); } } return wrapper; }
   * 
   * private static Element appendElement(Element wrapper, String field, String
   * label, String[] values) { if(values!=null && values.length>0){ Element el =
   * wrapper.appendElement("span").attr("field", field);
   * el.appendElement("span").attr("type", "label").text(label); Element
   * valuesWrapper = el.appendElement("span").attr("type", "values"); for
   * (String string : values) { valuesWrapper.appendElement("span").attr("type",
   ...
   * 
   * private static Element appendElement(Element wrapper, String field, String
   * label, Set<RepresentationState> values) { if(values!=null &&
   * values.size()>0){ Element el = wrapper.appendElement("span").attr("field",
   * field); el.appendElement("span").attr("type", "label").text(label); Element
   * valuesWrapper = el.appendElement("span").attr("type", "values"); for
