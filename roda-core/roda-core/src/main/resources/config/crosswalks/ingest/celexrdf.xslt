...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:cdm="http://publications.europa.eu/ontology/cdm#"
	...
	xmlns:case="mynamespace/case" xmlns:case-event="mynamespace/case-event"
	xmlns:celex="mynamespace/celex" xmlns:numpub="mynamespace/numpub"
	xmlns:agent="mynamespace/agent" xmlns:xsd="http://www.w3.org/2001/XMLSchema#"
	xmlns:rdfs="http://www.w3.org/2000/01/rdf-schema#" xmlns:rdf="http://www.w3.org/1999/02/22-rdf-syntax-ns#"
	xmlns:owl="http://www.w3.org/2002/07/owl#" xmlns:j1="http://www.w3.org/2004/02/skos/core#"
	...
	<xsl:template match="cdm:work_date_document">
		<xsl:if test="not(normalize-space(.)='')">
			...
			<field name="dateInitial">
				<xsl:value-of
					select="format-date($dateInitial,'[Y0001]-[M01]-[D01]T00:00:00Z')" />
			</field>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="cdm:work_date_creation_legacy">
		<xsl:if test="not(normalize-space(.)='')">
			...
			<field name="work_date_creation_legacy_dts">
				<xsl:value-of
					select="format-date($dateFinal,'[Y0001]-[M01]-[D01]T00:00:00Z')" />
			</field>
		</xsl:if>
	</xsl:template>

	<xsl:template match="cdm:expression_uses_language">
		<field name="expression_uses_language_txt">
	...
	<xsl:template
		match="cdm:expression_case-law_indicator_decision[@xml:lang='en']">
	...
	<xsl:template
		match="rdf:Description[rdf:type/@rdf:resource='http://publications.europa.eu/ontology/cdm#expression'][cdm:expression_uses_language/@rdf:resource='http://publications.europa.eu/resource/authority/language/ENG']/*[not(name(.)='cdm:expression_title')and(not(name(.)='cdm:expression_case-law_indicator_decision'))]">
		<xsl:if test="not(normalize-space(.)='')">
