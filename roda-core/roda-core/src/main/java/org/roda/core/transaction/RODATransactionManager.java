...
public class RODATransactionManager {
  ...
  private void processPluginExecutionResult(Plugin<IsRODAObject> plugin, UUID transactionId, Date initDate)
    ...
    try {
      ...
      String noRollback = plugin.getParameterValues().getOrDefault(RodaConstants.PLUGIN_PARAM_SKIP_ROLLBACK_ON_VALIDATION_FAILURE, "");
      Set<String> noRollbackPlugins = Arrays.stream(noRollback.split(",")).map(String::trim).filter(s -> !s.isEmpty()).collect(Collectors.toSet());
      boolean shouldRollback = false;
      for (Report failedReport : failedReports) {
        for (Report nestedReport: failedReport.getReports()){
          if (nestedReport.getPluginState().equals(PluginState.FAILURE) && !noRollbackPlugins.contains(nestedReport.getPlugin())) {
