...
public class RiskIncidenceCollection extends AbstractSolrCollection<RiskIncidence, RiskIncidence> {
  ...
  public List<Field> getFields() {
    ...
    fields.add(new Field(RodaConstants.RISK_INCIDENCE_DESCRIPTION, Field.TYPE_STRING));
    ...
    fields.add(new Field(RodaConstants.RISK_INCIDENCE_MITIGATED_DESCRIPTION, Field.TYPE_STRING));
  ...
  @Override
  public SolrInputDocument toSolrDocument(RiskIncidence incidence, Map<String, Object> preCalculatedFields,
    Map<String, Object> accumulators, Flags... flags)
    throws RequestNotValidException, GenericException, NotFoundException, AuthorizationDeniedException {

    SolrInputDocument doc = super.toSolrDocument(incidence, preCalculatedFields, accumulators, flags);
  ...
  public RiskIncidence fromSolrDocument(SolrDocument doc, List<String> fieldsToReturn) throws GenericException {
    ...
    if (doc.containsKey(RodaConstants.RISK_INCIDENCE_STATUS)) {
      incidence.setStatus(RiskIncidence.INCIDENCE_STATUS.valueOf(SolrUtils.objectToString(
        doc.get(RodaConstants.RISK_INCIDENCE_STATUS), RiskIncidence.INCIDENCE_STATUS.UNMITIGATED.toString())));
    }
    if (doc.containsKey(RodaConstants.RISK_INCIDENCE_SEVERITY)) {
      incidence.setSeverity(Risk.SEVERITY_LEVEL.valueOf(SolrUtils
