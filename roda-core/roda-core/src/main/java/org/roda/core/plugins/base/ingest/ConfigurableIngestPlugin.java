...
public class ConfigurableIngestPlugin extends DefaultIngestPlugin {
  ...
  public List<PluginParameter> getParameters() {
    ...
    if (!deactivatedPlugins.contains(PLUGIN_CLASS_VERAPDF)) {
      pluginParameters.add(getPluginParameter(PLUGIN_PARAMS_DO_VERAPDF_CHECK));
    }

    if (!deactivatedPlugins.contains(PLUGIN_CLASS_TIKA_FULLTEXT)) {
      pluginParameters.add(getPluginParameter(PLUGIN_PARAMS_DO_FEATURE_EXTRACTION));
      pluginParameters.add(getPluginParameter(PLUGIN_PARAMS_DO_FULL_TEXT_EXTRACTION));
    }

    if (!deactivatedPlugins.contains(PLUGIN_CLASS_DIGITAL_SIGNATURE)) {
      pluginParameters.add(getPluginParameter(PLUGIN_PARAMS_DO_DIGITAL_SIGNATURE_VALIDATION));
    }
  ...
  private void loadMap() {
    if (pluginParameters.isEmpty()) {
      ...
      Plugin<?> plugin = pluginManager.getPlugin(PLUGIN_CLASS_VERAPDF);
      if (plugin != null) {
        pluginParameters.put(PLUGIN_PARAMS_DO_VERAPDF_CHECK,
          new PluginParameter(PLUGIN_PARAMS_DO_VERAPDF_CHECK, plugin.getName(),
            PluginParameterType.BOOLEAN, "false", true, false, plugin.getDescription()));
      } else {
        deactivatedPlugins.add(PLUGIN_CLASS_VERAPDF);
      }
      ...
      plugin = pluginManager.getPlugin(PLUGIN_CLASS_TIKA_FULLTEXT);
      if (plugin != null) {
        pluginParameters.put(PLUGIN_PARAMS_DO_FEATURE_EXTRACTION,
          new PluginParameter(PLUGIN_PARAMS_DO_FEATURE_EXTRACTION, "Feature extraction",
            PluginParameterType.BOOLEAN, "false", true, false, "Extraction of technical metadata using Apache Tika"));

        pluginParameters.put(PLUGIN_PARAMS_DO_FULL_TEXT_EXTRACTION,
          new PluginParameter(PLUGIN_PARAMS_DO_FULL_TEXT_EXTRACTION, "Full-text extraction",
            PluginParameterType.BOOLEAN, "false", true, false, "Extraction of full-text using Apache Tika"));
      } else {
        deactivatedPlugins.add(PLUGIN_CLASS_TIKA_FULLTEXT);
      }

      plugin = pluginManager.getPlugin(PLUGIN_CLASS_DIGITAL_SIGNATURE);
      if (plugin != null) {
        pluginParameters.put(PLUGIN_PARAMS_DO_DIGITAL_SIGNATURE_VALIDATION,
          new PluginParameter(PLUGIN_PARAMS_DO_DIGITAL_SIGNATURE_VALIDATION, plugin.getName(),
            PluginParameterType.BOOLEAN, "false", true, false, plugin.getDescription()));
      } else {
        deactivatedPlugins.add(PLUGIN_CLASS_DIGITAL_SIGNATURE);
      }
  ...
  public void setTotalSteps() {
    this.totalSteps = INITIAL_TOTAL_STEPS - deactivatedPlugins.size();
  }
