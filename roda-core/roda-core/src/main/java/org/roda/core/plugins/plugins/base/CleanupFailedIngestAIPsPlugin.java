...
public class CleanupFailedIngestAIPsPlugin extends AbstractPlugin<Void> {
  ...
  @Override
  public Report execute(IndexService index, ModelService model, StorageService storage, List<Void> objects)
  ...
  private List<String> findActiveJobs(IndexService index) {
    ...
    index.findAll(Job.class, activeJobsViaStateFilter).forEach(e -> activeJobsIds.add(e.getId()));
