...
public class DeleteRodaObjectPluginUtils {
  ...
  private static void deleteFile(File file, IndexService index, ModelService model, Report report,
    ...
    Job job, boolean topFile) throws AuthorizationDeniedException, RequestNotValidException, GenericException {
    PluginState state = PluginState.SUCCESS;
    try {
      ...
      state = PluginState.FAILURE;
      reportItem.addPluginDetails("Could not delete children FILE: " + e.getMessage());
    }
    ...
    if (!topFile) {
      model.createEvent(file.getId(), null, null, null, RodaConstants.PreservationEventType.DELETION, EVENT_DESCRIPTION,
  ...
  private static void processFile(IndexService index, ModelService model, Report report, JobPluginInfo jobPluginInfo,
    ...
    try {
      ...
      if (StringUtils.isNotBlank(retrievedAIP.getDisposalScheduleId())) {
              ...
              public void run(IndexedFile item)
                ...
                try {
                  ...
                  File childFile = model.retrieveFile(item.getAipId(), item.getRepresentationId(), item.getPath(),
                    item.getId());
                  deleteFile(childFile, index, model, report, jobPluginInfo, reportItem, plugin, details, job, false);
        ...
        deleteFile(file, index, model, report, jobPluginInfo, reportItem, plugin, details, job, true);
      }
    } catch (RequestNotValidException | NotFoundException | GenericException | AuthorizationDeniedException e) {
      state = PluginState.FAILURE;
