 ...
 */
public class AkkaBackgroundJobActor extends AkkaBaseActor {

  private static final Logger LOGGER = LoggerFactory.getLogger(AkkaBackgroundJobActor.class);

  private ActorRef jobsManager;

  private SupervisorStrategy strategy = new OneForOneStrategy(false, DeciderBuilder.matchAny(e -> {
    LOGGER.error("A child actor of {} has thrown an exception", AkkaBackgroundJobActor.class.getSimpleName(), e);
  ...
  /** Public constructor */
  public AkkaBackgroundJobActor(ActorRef jobsManager) {
    super();
