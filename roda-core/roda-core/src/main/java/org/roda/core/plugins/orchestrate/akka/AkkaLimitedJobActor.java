 ...
 */
public class AkkaLimitedJobActor extends AkkaBaseActor {
  private static final Logger LOGGER = LoggerFactory.getLogger(AkkaLimitedJobActor.class);

  private ActorRef jobsManager;

  private SupervisorStrategy strategy = new OneForOneStrategy(false, DeciderBuilder.matchAny(e -> {
    LOGGER.error("A child actor of {} has thrown an exception", AkkaLimitedJobActor.class.getSimpleName(), e);
  ...
  /** Public constructor */
  public AkkaLimitedJobActor(ActorRef jobsManager) {
    super();
