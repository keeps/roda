...
public class TransactionalStorageServiceTest extends AbstractStorageServiceTest<TransactionalStorageService> {
  ...
  public void testCreateRollback() throws RODATransactionException, AuthorizationDeniedException,
    ...
    // 1.4) create another binary directly through the main storage
    final StoragePath fakeContainerStoragePath = StorageTestUtils.generateRandomContainerStoragePath();
    final StoragePath fakeBinaryStoragePath = StorageTestUtils.generateRandomResourceStoragePathUnder(fakeContainerStoragePath);
    // 1.4) update the content of a binary that doesn't exist to make the
    // transaction fail on commit
    storage1.updateBinaryContent(fakeBinaryStoragePath, payload1, false, false);
    try {
      // 1.5) end first transaction, this should fail
      transactionManager.endTransaction(context1.transactionLog().getId());
      Assert.fail("An exception should have been thrown due to trying to update the content of a non existent binary");
    ...
    TransactionalContext context2 = transactionManager.beginTestTransaction(mainStorage);
    StorageService storage2 = context2.transactionalStorageService();
    // 2.2) test that the container doesn't exist
