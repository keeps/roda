...
public class Ingest extends RODAWebService {
  ...
  public String createDetachedDescriptionObject(DescriptionObject dObject) throws NoSuchRODAObjectException,
    ...
    registerAction("Ingest.createDetachedDescriptionObject", new String[] {"descriptionObject", dObject.toString()},
      "User %username% called method Ingest.createDetachedDescriptionObject(" + dObject + ")", duration);
  ...
  public String createDescriptionObject(DescriptionObject dObject) throws NoSuchRODAObjectException,
    ...
    registerAction("Ingest.createDescriptionObject", new String[] {"descriptionObject", dObject.toString()},
      "User %username% called method Ingest.createDescriptionObject(" + dObject + ")", duration);
  ...
  public String createDetachedDescriptionObjectWithOtherMetadata(DescriptionObject dObject,
    String otherMetadataFilePath) throws NoSuchRODAObjectException, InvalidDescriptionObjectException,
    IngestException {
    Date start = new Date();

    String pid = getIngestHelper().createDetachedDescriptionObject(dObject, otherMetadataFilePath);
    long duration = new Date().getTime() - start.getTime();

    registerAction("Ingest.createDetachedDescriptionObjectWithOtherMetadata", new String[] {"descriptionObject",
      dObject.toString(), "otherMetadataFilePath", otherMetadataFilePath},
      "User %username% called method Ingest.createDetachedDescriptionObjectWithOtherMetadata(" + dObject + ","
        + otherMetadataFilePath + ")", duration);
   ...
   */
  // FIXME revise javadocs
  public String createDescriptionObjectWithOtherMetadata(DescriptionObject dObject, String otherMetadataFilePath)
    throws NoSuchRODAObjectException, InvalidDescriptionObjectException, IngestException {
    Date start = new Date();

    String pid = getIngestHelper().createDescriptionObject(dObject, otherMetadataFilePath);
    long duration = new Date().getTime() - start.getTime();

    registerAction("Ingest.createDescriptionObjectWithOtherMetadata",
      new String[] {"descriptionObject", dObject.toString(), "otherMetadataFilePath", otherMetadataFilePath},
      "User %username% called method Ingest.createDescriptionObjectWithOtherMetadata(" + dObject + ","
        + otherMetadataFilePath + ")", duration);
  ...
  public void removeDescriptionObject(String doPID) throws NoSuchRODAObjectException, IngestException {
    ...
    try {
      ...
      registerAction("Ingest.removeDescriptionObject", new String[] {"doPID", doPID},
        "User %username% called method Ingest.removeDescriptionObject(" + doPID + ")", duration);
   ...
   * properties, the {@link RepresentationFile}s are not created by this method.
   * Use FileUpload servlet to add {@link RepresentationFile}s to the
   ...
   * @throws IngestException
   */
  public String createRepresentationObject(RepresentationObject rObject) throws IngestException,
    ...
    try {
      ...
      registerAction("Ingest.createRepresentationObject", new String[] {"rObject", rObject.toString()},
        "User %username% called method Ingest.createRepresentationObject(" + rObject + ")", duration);
  ...
  public String createRepresentationPreservationObject(RepresentationPreservationObject rpo, String doPID)
    throws NoSuchRODAObjectException, RepresentationAlreadyPreservedException, IngestException {

    Date start = new Date();

    try {

      String pid = getIngestHelper().createRepresentationPreservationObject(rpo, doPID);
      long duration = new Date().getTime() - start.getTime();

      registerAction("Ingest.createRepresentationPreservationObject", new String[] {"rpo", rpo.toString(), "doPID",
        doPID}, "User %username% called method Ingest.createRepresentationPreservationObject(" + rpo + ", " + doPID
        + ")", duration);
  ...
  public String setDONormalizedRepresentation(String doPID, String roPID) throws NoSuchRODAObjectException,
    ...
    try {
      ...
      String pid = getIngestHelper().setDONormalizedRepresentation(doPID, roPID);
      long duration = new Date().getTime() - start.getTime();

      registerAction("Ingest.setDONormalizedRepresentation", new String[] {"doPID", doPID, "roPID", roPID},
        "User %username% called method Ingest.setDONormalizedRepresentation(" + doPID + ", " + roPID + ")", duration);
  ...
  public void createDerivationRelationship(String rpoPID, String derivationEventPID) throws IngestException {
    ...
    try {
      ...
      getIngestHelper().createDerivationRelationship(rpoPID, derivationEventPID);
      long duration = new Date().getTime() - start.getTime();

      registerAction("Ingest.createDerivationRelationship", new String[] {"rpoPID", rpoPID, "derivationEventPID",
        derivationEventPID}, "User %username% called method Ingest.createDerivationRelationship(" + rpoPID + ", "
        + derivationEventPID + ")", duration);
  ...
  public String registerIngestEvent(String[] doPIDs, String[] roPIDs, String[] poPIDs, String agentName, String details)
    ...
    try {
      ...
      String pid = getIngestHelper().registerIngestEvent(doPIDs, roPIDs, poPIDs, agentName, details);
      long duration = new Date().getTime() - start.getTime();

      registerAction(
        "Ingest.registerIngestEvent",
        new String[] {"doPIDs", Arrays.toString(doPIDs), "roPIDs", Arrays.toString(roPIDs), "poPIDs",
          Arrays.toString(poPIDs), "agentName", agentName, "details", details},
        "User %username% called method Ingest.registerIngestEvent(" + Arrays.toString(doPIDs) + ", "
          + Arrays.toString(roPIDs) + ", " + Arrays.toString(poPIDs) + ", " + agentName + ", " + details + ")",
        duration);
  ...
  public String registerEvent(String rpoPID, EventPreservationObject eventPO, AgentPreservationObject agent)
    ...
    try {
      ...
      String pid = getIngestHelper().registerEvent(rpoPID, eventPO, agent);
      long duration = new Date().getTime() - start.getTime();

      registerAction("Ingest.registerEvent", new String[] {"rpoPID", rpoPID, "eventPO", "" + eventPO, "agent",
        "" + agent}, "User %username% called method Ingest.registerIngestion(" + rpoPID + ", " + eventPO + ", " + agent
        + ")", duration);
  ...
  public String registerDerivationEvent(String originalRepresentationPID, String derivedRepresentationPID,
    ...
    try {
      ...
      String pid = getIngestHelper().registerDerivationEvent(originalRepresentationPID, derivedRepresentationPID,
        eventPO, agentPO, markObjectsActive);
      long duration = new Date().getTime() - start.getTime();

      registerAction("Ingest.registerDerivationEvent", new String[] {"originalRepresentationPID",
        originalRepresentationPID, "derivedRepresentationPID", derivedRepresentationPID, "eventPO", "" + eventPO,
        "agentPO", "" + agentPO}, "User %username% called method Ingest.registerDerivationEvent("
        + originalRepresentationPID + ", " + derivedRepresentationPID + ", " + eventPO + ", " + agentPO + ")", duration);
  ...
  public String[] removeObjects(String[] pids) throws IngestException {
    ...
    String[] result = removedObjects.toArray(new String[removedObjects.size()]);
    long duration = new Date().getTime() - start.getTime();

    registerAction("Ingest.removeObjects", new String[] {"pids", Arrays.toString(pids)},
      "User %username% called method Ingest.removeObjects(" + Arrays.asList(pids) + ")", duration);

    return result;
  }

  // FIXME add javadocs
  // FIXME register action
  public String[] getPossibleParents(String producerUsername, String classificationSchemeId, String classFullId)
    throws IngestException {

    logger.info("Going to get possibleParents from fedoraGSearch");
    // search Lucene for classes with provided classFullId
    String[] possibleParentsPids = getIngestHelper().getPossibleParents(getClientUser(),
      getConfiguration().getString("fedoraGSearchURL"), classFullId);
    logger.info("Result: " + Arrays.toString(possibleParentsPids));

    // determine which classes have as an ancestor a classification_scheme with
    // provided classificationSchemeId and in which the producer can ingest into
    return getIngestHelper().getPossibleParentsByClassificationSchemeId(producerUsername, classificationSchemeId,
      possibleParentsPids);
  }

  // FIXME add javadocs
  // FIXME register action
  public DescriptionObject findVolumeParent(String classPID, String caseFileFullId, String subfileFullId)
    throws IngestException {
    return getIngestHelper().findVolumeParent(getClientUser(), getConfiguration().getString("fedoraGSearchURL"),
      classPID, caseFileFullId, subfileFullId);
  }

  private IngestHelper getIngestHelper() throws IngestException {

    CASUserPrincipal clientUser = getClientUser();
    IngestHelper ingestHelper = null;
    if (clientUser != null) {

      try {
        String fedoraURL = getConfiguration().getString("fedoraURL");
        String fedoraGSearchURL = getConfiguration().getString("fedoraGSearchURL");
        ...
        ingestHelper = new IngestHelper(fedoraClientUtility, getConfiguration());

      } catch (FedoraClientException e) {
        logger.debug("Error creating Fedora client - " + e.getMessage(), e);
        throw new IngestException("Error creating Fedora client - " + e.getMessage(), e);
      } catch (EditorException e) {
        logger.debug("Error creating Fedora client - " + e.getMessage(), e);
        throw new IngestException("Error creating Fedora client - " + e.getMessage(), e);
      } catch (MalformedURLException e) {
        logger.debug("Bad URL for Fedora client - " + e.getMessage(), e);
        throw new IngestException("Bad URL for Fedora client - " + e.getMessage(), e);
      }
