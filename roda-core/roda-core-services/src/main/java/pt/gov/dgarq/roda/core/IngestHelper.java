...
public class IngestHelper extends EditorHelper {
     ...
     */
    public IngestHelper(FedoraClientUtility fedoraClientUtility,
     ...
     *
     * @param dObject the {@link DescriptionObject} to create.
     *
     * @return a {@link String} with the PID of the newly created object.
     *
     * @throws NoSuchRODAObjectException if the * * * * * *
     * specified <code>doParentPID</code> doesn't exist.
     * @throws InvalidDescriptionObjectException if the * * * * * *
     * specified <code>dObject</code> is invalid.
        ...
        try {
            ...
            if (doPID != null) {
                ...
                try {
                    ...
                    logger.error("Error removing created DO " + doPID + " - "
                            + e1.getMessage() + ". IGNORING");
                }
            }

            throw new IngestException("Error creating detached DO - "
                    + e.getMessage(), e);
        }
    ...
    /**
     * Removes the {@link DescriptionObject} with PID
     * <code>pid</code> and all it's descendants as long as the
     * {@link DescriptionObject} is inactive.
     *
     * @param doPID the PID of the {@link DescriptionObject} to remove.
     *
     * @throws NoSuchRODAObjectException
     * @throws IngestException
     */
    public void removeDescriptionObject(String doPID)
        ...
        try {
            ...
            logger.debug("Exception getting DO " + doPID + " - "
                    + e.getMessage(), e);
            throw new IngestException("Exception getting DO " + doPID + " - "
                    + e.getMessage(), e);
        }
        ...
        if (sdo.getState().equals(RODAObject.STATE_INACTIVE)) {
            ...
            logger.warn("DO " + doPID
                    + " cannot be removed because it is active.");
            throw new IngestException("DO " + doPID
                    + " cannot be removed because it is active.");
        }
        ...
        try {
            ...
            logger.debug("Exception getting descendants of DO " + doPID + " - "
                    + e.getMessage(), e);
            throw new IngestException("Exception getting descendants of DO "
                    + doPID + " - " + e.getMessage(), e);
        }
        ...
        try {
            ...
            logger.debug("Error removing DO " + doPID + " - " + e.getMessage(),
                    e);
            throw new IngestException("Error removing DO " + doPID + " - "
                    + e.getMessage(), e);
        }
    ...
    /**
     * @param roObject the {@link RepresentationObject} to create.
     *
     ...
     * @throws EditorException
     */
    public String createRepresentationObject(RepresentationObject roObject)
            throws NoSuchRODAObjectException, EditorException {

        if (roObject == null) {
            throw new EditorException(
                    "representation object argument cannot be null");
        }
        ...
        try {
            simpleDO = getBrowserHelper().getSimpleDescriptionObject(
                    roObject.getDescriptionObjectPID());
        ...
        try {
            ...
            try {
                ...
                throw new EditorException(
                        "Exception getting permissions of RO "
                        + roObject.getDescriptionObjectPID() + " - "
                        + e.getMessage(), e);
            }
            ...
            getFedoraClientUtility().getAPIM().addDatastream(pid,
                    this.relsExtDatastreamID, new String[0],
                    "Relationship Metadata", true, "text/xml", null,
                    temporaryURL, "X", "A", null, null,
            ...
            temporaryURL = getFedoraClientUtility().temporaryUpload(
                    policyByteArray);
            ...
            getFedoraClientUtility().getAPIM().addDatastream(
                    ...
                    getBrowserHelper().policyDatastreamID, new String[0],
                    "XACML Policy", true, "text/xml", null, temporaryURL, "X",
                    "A", null, null, "Added by RODA Core Services");
            ...
            getFedoraClientUtility().addRepresentedByRelationship(
                    roObject.getDescriptionObjectPID(), pid);

            return pid;

        } catch (Exception e) {

            logger.debug("Exception creating representation object - "
                    + e.getMessage(), e);
            ...
            throw new EditorException(
                    "Exception creating representation object - "
                    + e.getMessage(), e);
        }
     ...
     *
     * @param rpo the {@link RepresentationPreservationObject} to create.
     ...
     *
     * @throws NoSuchRODAObjectException if one of the given PIDs (doPID or
     * roPID) doesn't exist.
     * @throws EditorException
     */
    public String setDONormalizedRepresentation(String doPID, String roPID)
        ...
        try {
            ...
            SimpleDescriptionObject simpleDO = getBrowserHelper()
                    .getSimpleDescriptionObject(doPID);

            logger.debug("setDONormalizedRepresentation(" + doPID + "," + roPID
                    + ") DO is " + simpleDO);

        } catch (BrowserException e) {
            logger.debug("Error getting DescriptionObject " + doPID + " - "
                    + e.getMessage(), e);
            throw new EditorException("Error getting DescriptionObject "
                    + doPID + " - " + e.getMessage(), e);
        }
        ...
        try {
            ...
            if (!simpleRO.getDescriptionObjectPID().equals(doPID)) {
                throw new EditorException(roPID
                        + " is not a representation of " + doPID);
            }
        ...
        try {
            ...
            Filter normROfilter = new Filter(new FilterParameter[]{
                new SimpleFilterParameter("descriptionObjectPID", doPID),
                new SimpleFilterParameter("statuses",
                RepresentationObject.STATUS_NORMALIZED)});
            List<SimpleRepresentationObject> normalizedSROs = getBrowserHelper()
                    .getSimpleRepresentationObjects(
                    new ContentAdapter(normROfilter, null, null));

            if (normalizedSROs.size() > 0) {

                if (normalizedSROs.size() > 1) {
                    logger.warn("DO " + doPID + " has " + normalizedSROs.size()
                ...
                for (SimpleRepresentationObject normalizedSRO : normalizedSROs) {
                    ...
                    if (normalizedSRO.equals(roPID)) {
                        ...
                        logger.info("Representation " + roPID
                                + " already has normalized status");
                    } else {
                        // This object is not the normalized representation.
                        logger
                                .info("Representation "
                                + roPID
                                + " is not the normalized representation. Removing status normalized");

                        normalizedSRO
                                .removeStatus(RepresentationObject.STATUS_NORMALIZED);

                        try {

                            getFedoraClientUtility()
                                    .setRepresentationProperties(
                                    normalizedSRO.getPid(),
                                    normalizedSRO.getStatuses(),
                                    normalizedSRO.getType(),
                                    normalizedSRO.getSubType());

                        } catch (FedoraClientException e) {
                            logger.error(
                                    "Error removing normalized status from RO "
                                    + normalizedSRO.getPid() + " - "
                                    + e.getMessage(), e);
                        }
                ...
                logger.info("Marking " + roPID
                        + " as the normalized representation of DO " + doPID);

                simpleRO.addStatus(RepresentationObject.STATUS_NORMALIZED);

                try {

                    getFedoraClientUtility().setRepresentationProperties(
                            simpleRO.getPid(), simpleRO.getStatuses(),
                            simpleRO.getType(), simpleRO.getSubType());

                } catch (FedoraClientException e) {
                    logger.debug("Error setting normalized status on RO "
                            + simpleRO.getPid() + " - " + e.getMessage(), e);
                    throw new EditorException(
                            "Error setting normalized status on RO "
                            + simpleRO.getPid() + " - "
                            + e.getMessage(), e);
                }
            }

            logger.info("Normalized representation of DO " + doPID + " is RO "
                    + roPID);

            return roPID;

        } catch (BrowserException e) {
            logger.debug("Error getting DescriptionObject's representations "
                    + " - " + e.getMessage(), e);
            throw new EditorException(
                    "Error getting DescriptionObject's representations "
                    + " - " + e.getMessage(), e);
        }
    ...
    public void createDerivationRelationship(String rpoPID,
        ...
        try {
            ...
            getFedoraClientUtility().addDerivedFromRelationship(rpoPID,
                    derivationEventPID);
            ...
            throw new IngestException(
                    "Error creating derivation relationship between " + rpoPID //$NON-NLS-1$
     ...
     *
     * @param doPIDs the PIDs of the {@link DescriptionObject}s.
     * @param roPIDs the PIDs of the {@link RepresentationObject}s.
     * @param poPIDs the PIDs of the {@link PreservationObject}s.
     * @param agentName the name of the agent responsible for the ingestion.
     * @param details the details of the ingest operation.
     *
     *
    ...
    public String registerIngestEvent(String[] doPIDs, String[] roPIDs,
        ...
        try {
            ...
            agent = getBrowserHelper().getAgentPreservationObjectWithName(
                    agentName);
            ...
            if (agent == null) {
                ...
                agent
                        .setAgentType(AgentPreservationObject.PRESERVATION_AGENT_TYPE_INGEST_TASK);
            ...
            logger.debug("Exception getting preservation agent -"
                    + e.getMessage(), e);
            throw new EditorException("Exception getting preservation agent -"
                    + e.getMessage(), e);
        }
        ...
        try {
            ...
            if (roPIDs != null) {
                ...
                for (String roPID : roPIDs) {
                    ...
                    if (rpo == null) {
                        ...
                        String rpoPID = createAndInsertRepresentationPreservationObject(
                                roPID, null, null);

                        logger.trace("Created preservation object " + rpoPID
                                + " for representation " + roPID);
                        ...
                        logger.info("Representation " + roPID
                                + " already has a preservation object ("
                                + rpo.getPid() + ")");
            ...
            EventPreservationObject ingestEvent = createIngestEvent(doPIDs,
                    roPIDs, poPIDs, details, agent.getID(), createdObjectPIDs);

            logger.trace("Ingestion Event is " + ingestEvent);

            String ingestEventPID = insertEventPreservationObject(ingestEvent,
                    rpoPIDs.toArray(new String[rpoPIDs.size()]), agent.getPid());
            ...
            logger.debug("Error registering ingestion - " + e.getMessage()
            ...
            throw new EditorException("Error registering ingestion - "
                    + e.getMessage(), e);
     ...
     * {@link RepresentationObject}.
     *
     * @param rpoPID the PIDs of the {@link RepresentationPreservationObject}.
     * @param eventPO the {@link EventPreservationObject}.
     * @param agent the {@link AgentPreservationObject}.
     *
     *
    ...
    public String registerEvent(String rpoPID, EventPreservationObject eventPO,
            AgentPreservationObject agent) throws EditorException {
        ...
        try {
            ...
            // Get or create Agent information
            existingAgent = getBrowserHelper()
                    .getAgentPreservationObjectWithName(agent.getAgentName());
        ...
        try {
            ...
            String eventPID = insertEventPreservationObject(eventPO,
                    new String[]{rpoPID}, existingAgent.getPid());
            ...
            logger.debug("Error registering event - " + e.getMessage()
                    + ". Removing created objects.");
            ...
            throw new EditorException("Error registering event - "
                    + e.getMessage(), e);
     ...
     * {@link RepresentationObject}.
     *
     * @param originalRepresentationPID the PID of the original
     * {@link RepresentationObject}.
     * @param derivedRepresentationPID the PID of the derived
     * {@link RepresentationObject}.
     * @param eventPO the {@link EventPreservationObject}.
     * @param agentPO the {@link AgentPreservationObject}.
     * @param markObjectsActive should the created objects be marked active?
     *
     *
     * @return the PID of the {@link EventPreservationObject}.
     *
    ...
    public String registerDerivationEvent(String originalRepresentationPID,
        ...
        try {
            ...
            logger
                    .info("registerMigrationEvent() performing sanity check, before registering event.");

            // Get original Representation
            originalRO = getBrowserHelper().getSimpleRepresentationObject(
                    originalRepresentationPID);
            logger.info("Original RO is " + originalRO);

            originalRPO = getBrowserHelper().getROPreservationObject(
                    originalRepresentationPID);
            logger.info("Original RPO is " + originalRPO);

            // Get derived Representation
            derivedRO = getBrowserHelper().getSimpleRepresentationObject(
                    derivedRepresentationPID);
            ...
            existingAgentPO = getBrowserHelper()
                    .getAgentPreservationObjectWithName(agentPO.getAgentName());
            if (existingAgentPO != null) {
                logger.info("Agent PO " + agentPO.getAgentName()
                        + " already exist " + existingAgentPO);
            } else {
                logger.info("Agent PO " + agentPO.getAgentName()
                        + " doesn't exist.");
            }
            ...
            if (StringUtils.isBlank(eventPO.getEventType())
                    || StringUtils.isBlank(eventPO.getOutcome())
                ...
                logger.warn("Event PO outcomeDetailNote: "
                        + eventPO.getOutcomeDetailNote());
                logger.warn("Event PO outcomeDetailNoteExtension: "
                        + eventPO.getOutcomeDetailExtension());

                throw new EditorException(
                        "Event outcome attributes must be filled.");
            }

        } catch (BrowserException e) {
            logger.debug("Error checking existing objects - " + e.getMessage(),
                    e);
            throw new EditorException("Error checking existing objects - "
                    + e.getMessage(), e);
        }
        ...
        eventPO.setEventDetail("The representation " + derivedRepresentationPID
                + " derived from representation " + originalRepresentationPID);
        ...
        String eventPID = insertEventPreservationObject(eventPO,
                new String[]{originalRPO.getPid()}, agentPID);
        ...
        try {
            ...
            String derivedRPOPID = createAndInsertRepresentationPreservationObject(
                    derivedRepresentationPID, eventPID, originalRPO.getID());
            createdObjectPIDs.add(derivedRPOPID);

            // Create relationship with derivation event
            getFedoraClientUtility().addDerivedFromRelationship(derivedRPOPID,
                    eventPID);

            logger.info("Created preservation object " + derivedRPOPID
                    + " for representation " + derivedRepresentationPID);

        } catch (Exception e) {

            logger.debug("Error creating representation preservation object - "
                    + e.getMessage() + ". Deleting created objects "
                    + createdObjectPIDs, e);

            removeObjects(createdObjectPIDs);

            throw new EditorException(
                    "Error creating representation preservation object - "
                    + e.getMessage(), e);
        }
        ...
        if (markObjectsActive) {
            try {
                ...
                getFedoraClientUtility().markObjectsActive(createdObjectPIDs,
                        true);

                logger.info("Marked objects " + createdObjectPIDs
                        + " as Active");

            } catch (FedoraClientException e) {

                logger.debug("Error marking objects as Active - "
                        + e.getMessage() + ". Deleting created objects "
                        + createdObjectPIDs, e);

                removeObjects(createdObjectPIDs);

                throw new EditorException("Error marking objects as Active - "
                        + e.getMessage(), e);
            }
        }
    ...
    private RepresentationPreservationObject createRepresentationPreservationObject(
            String roPID) throws NoSuchRODAObjectException, BrowserException,
            EditorException {
        ...
        if (RepresentationObject.UNKNOWN.equals(rObject.getType())) {
            pObject
                    .setPreservationLevel(RepresentationPreservationObject.PRESERVATION_LEVEL_BITLEVEL);
        } else {
            pObject
                    .setPreservationLevel(RepresentationPreservationObject.PRESERVATION_LEVEL_FULL);
        }

        RepresentationFilePreservationObject pObjectRootFile = createRepresentationFilePreservationObject(
                rObject.getPid(), rObject.getRootFile());
        ...
        pObject
                .setPartFiles(pObjectPartFiles
                .toArray(new RepresentationFilePreservationObject[pObjectPartFiles
                .size()]));

        return pObject;
    }

    private RepresentationFilePreservationObject createRepresentationFilePreservationObject(
            String roPID, RepresentationFile rFile) throws EditorException {
        ...
        // <preservationLevel>
        pObjectFile
                .setPreservationLevel(RepresentationFilePreservationObject.PRESERVATION_LEVEL_FULL);
        ...
        try {
            ...
            logger.debug("Error calculating datastream checksum - "
                    + e.getMessage(), e);
            throw new EditorException(
                    "Error calculating datastream checksum - " + e.getMessage(),
                    e);
        } catch (IOException e) {
            logger.debug("Error calculating datastream checksum - "
                    + e.getMessage(), e);
        ...
        String objectCharacteristicsExtension = getPreservationFileObjectCharacteristicsExtension(
                roPID, rFile);
        pObjectFile
        ...
        pObjectFile
                .setContentLocationType("RODAObjectPID/RODAObjectDatastreamID");
        pObjectFile.setContentLocationValue(roPID + "/" + rFile.getId());

        return pObjectFile;
    }

    private String getPreservationFileObjectCharacteristicsExtension(
            String roPID, RepresentationFile rFile) throws EditorException {

        try {

            // Get datastream from Fedora
            InputStream inputStream = getFedoraClientUtility().getDatastream(
                    roPID, rFile.getId());
            // Create a temporary file to save the datastream
            File dsTempFile = File.createTempFile("rFile", "temp");
            FileOutputStream dsTempFileOutputStream = new FileOutputStream(
                    dsTempFile);
            ...
            // Inspect the file with Jhove utility
            String jhoveOutput = JhoveUtility.inspect(dsTempFile);
            ...
            logger.debug("Error inspecting representation file " + roPID + "/"
                    + rFile.getId() + " - " + e.getMessage(), e);
            throw new EditorException("Error inspecting representation file "
                    + roPID + "/" + rFile.getId() + " - " + e.getMessage(), e);
        }
    }

    private EventPreservationObject createIngestEvent(String[] doPIDs,
            String[] roPIDs, String[] poPIDs, String details, String agentPID,
            List<String> createdRepPObjectPIDs) {
        ...
        event
                .setEventDetail("The objects inside the SIP were added to the preservation repository");
        ...
        event.setObjectIDs(createdRepPObjectPIDs
                .toArray(new String[createdRepPObjectPIDs.size()]));

        return event;
    }

    /**
     * Create preservation metadata for the specified representation and inserts
     * a new preservation object.
     *
     * @param roPID the PID of the {@link RepresentationObject}.
     ...
     * @throws EditorException
     */
    private String createAndInsertRepresentationPreservationObject(
            String roPID, String derivationEventID,
            String derivedFromRepresentationID) throws EditorException,
            FedoraRISearchException, NoSuchRODAObjectException,
        ...
        repPreservationObject
                .setDerivedFromRepresentationObjectID(derivedFromRepresentationID);

        repPreservationObject.setState(RODAObject.STATE_INACTIVE);
        repPreservationObject.setRepresentationObjectPID(roPID);

        return insertRepresentationPreservationObject(repPreservationObject,
                null);
    }

    private String insertRepresentationPreservationObject(
            RepresentationPreservationObject rpo, String doPID)
            throws NoSuchRODAObjectException,
            RepresentationAlreadyPreservedException, EditorException {
        ...
        if (roPID == null) {
            // RPO is orphan
            logger
                    .debug("RPO representation object PID is null. RPO is orphan.");
        } else {
            ...
            try {
                ...
                logger.debug("Error getting representation object " + roPID
                        + " - " + e.getMessage(), e);
                throw new EditorException(
                        "Error getting representation object " + roPID + " - "
                        + e.getMessage(), e);
            }

            try {

                simpleRPO = getFedoraClientUtility().getFedoraRISearch()
                        .getROPreservationObject(roPID);

            } catch (FedoraRISearchException e) {
                logger.debug(
                        "Error getting representation preservation object "
                        + roPID + " - " + e.getMessage(), e);
                throw new EditorException(
                        "Error getting representation preservation object "
                        + roPID + " - " + e.getMessage(), e);
            }
        ...
        if (simpleRPO == null) {
            ...
            try {
                ...
                // to Fedora
                byte[] serializedRepresentationPremis = new PremisRepresentationObjectHelper(
                        rpo).saveToByteArray();

                // Add representation PREMIS metadata
                addPREMISDatastream(poPID, this.preservationObjectDatastreamID,
                ...
                if (rpo.getPartFiles() != null) {
                    ...
                    for (RepresentationFilePreservationObject partFile : rpo
                            .getPartFiles()) {
                ...
                logger.error(
                        "Exception creating preservation object - "
                        + e.getMessage() + ". Deleting created object "
                        + poPID, e);
            ...
            try {
                ...
                logger.debug("Error copying permissions from " + roPID + " to "
                        + poPID + ". Deleting created object " + poPID, e);

                removeObjects(new String[]{poPID});

                throw new EditorException("Error copying permissions from "
                        + roPID + " to " + poPID + " - " + e.getMessage(), e);
            }
            ...
            try {
                ...
                // poPID);
                getFedoraClientUtility().addPreservationOfRelationship(poPID,
                        roPID);

            } catch (Exception e) {

                logger.debug("Exception creating relationship between " + roPID
                        + " and " + poPID + ". Deleting created object "
                        + poPID, e);

                removeObjects(new String[]{poPID});

                throw new EditorException(
                        "Exception creating relationship between " + roPID
                        + " and " + poPID + " - " + e.getMessage(), e);
            }
            ...
            logger.debug("RepresentationObject " + roPID
                    + " already has a PreservationObject ("
                    + simpleRPO.getPid() + ")");
            throw new RepresentationAlreadyPreservedException(
                    "RepresentationObject " + roPID
                    + " already has a PreservationObject ("
                    + simpleRPO.getPid() + ")");
    ...
    private String insertEventPreservationObject(EventPreservationObject event,
            String[] performedOnPIDs, String agentPID) throws EditorException {
        ...
        try {
            ...
            // Transforms the Preservation Object into a byte array to send
            // to Fedora
            byte[] serializedPremisEvent = new PremisEventHelper(event)
                    .saveToByteArray();

            // Add representation PREMIS metadata
            addPREMISDatastream(poPID, this.preservationObjectDatastreamID,
                    "Event Preservation Metadata", serializedPremisEvent);

            // Create relationships with objects and agent
            getFedoraClientUtility().addPerformedOnRelationships(poPID,
                    performedOnPIDs);
            getFedoraClientUtility()
                    .addPerformedByRelationship(poPID, agentPID);

            return poPID;

        } catch (Exception e) {

            logger.error("Exception creating preservation object - "
                    + e.getMessage() + ". Deleting object " + poPID, e);
    ...
    private String insertAgentPreservationObject(AgentPreservationObject agent)
            throws EditorException {
        ...
        if (!AgentPreservationObject.TYPE.equals(agent.getType())) {
            throw new EditorException("agent contentModel type is "
                    + agent.getType() + " and should be "
        ...
        if (!Arrays.asList(AgentPreservationObject.PRESERVATION_AGENT_TYPES)
                .contains(agent.getAgentType())) {
            ...
            logger
                    .warn("Agent type '"
                    + agent.getAgentType()
                    + "' is unknown. Setting agent type to "
                    + AgentPreservationObject.PRESERVATION_AGENT_TYPE_UNKNOWN_PREFIX
                    + agent.getAgentType());
            agent
                    .setAgentType(AgentPreservationObject.PRESERVATION_AGENT_TYPE_UNKNOWN_PREFIX
                    + agent.getAgentType());
        }
        ...
        try {
            ...
            byte[] serializedRepresentationPremis = premisHelper
                    .saveToByteArray();

            // Add representation PREMIS metadata
            addPREMISDatastream(poPID, this.preservationObjectDatastreamID,
                    "Agent Preservation Metadata",
                    serializedRepresentationPremis);
    ...
    private void addFilePremisDatatream(String poPID,
            RepresentationFilePreservationObject rFile)
            throws PremisMetadataException, FedoraClientException,
            RemoteException {

        logger.debug("Adding RepresentationFilePreservationObject " + rFile);

        PremisFileObjectHelper premisFileObjectHelper = new PremisFileObjectHelper(
                rFile);
        logger.debug("Adding PREMIS object "
                + premisFileObjectHelper.getObjectDocument());
    ...
    private void addPREMISDatastream(String poPID, String dsID, String dsLabel,
            byte[] datastream) throws FedoraClientException, RemoteException {
        ...
        getFedoraClientUtility().getAPIM().addDatastream(poPID, dsID,
                new String[0], dsLabel, true, "text/xml", null, temporaryURL,
                "M", "A", null, null, "Added by RODA Core Services");

        logger.info("Added datastream " + dsID + " to object " + poPID);
    }

    private Fixity calculateFixity(String roPID, RepresentationFile rFile,
            String digestAlgorithm) throws IOException,
            NoSuchAlgorithmException {

        InputStream dsInputStream = getFedoraClientUtility().getDatastream(
                roPID, rFile.getId());

        Fixity fixity = new Fixity(digestAlgorithm, FileUtility
                .calculateChecksumInHex(dsInputStream, digestAlgorithm),
                "RODA Core Services");
