...
public class SearchTest {
  ...
  private static void showResults(SearchResult search) {
    ...
    if (search == null) {
      ...
      logger.info("SearchResult (" + search.getDatetime() + ", " + search.getIndexName() + ", " + search.getHitTotal()
        + ", " + search.getHitPageSize() + ", " + search.getHitPageStart() + ", " + search.getResultCount()
        + " results)");

      SearchResultObject[] searchResults = search.getSearchResultObjects();
      ...
      if (searchResults == null || searchResults.length == 0) {
        ...
        logger.info("query returned " + searchResults.length + " results");
