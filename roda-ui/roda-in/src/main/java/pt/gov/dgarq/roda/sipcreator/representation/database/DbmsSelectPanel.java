			...
			if (file.getName().equals(DBMLImportModule.DBML_DEFAULT_FILE_NAME)) {
				try {
					rootFile = new RepresentationFile("F0", file.getName(),
							RepresentationBuilder.getFileMimetype(file), file
									.length(), file.toURI().toURL().toString());
				...
				try {
					RepresentationFile partFile = new RepresentationFile(file
							.getName(), file.getName(), RepresentationBuilder
							.getFileMimetype(file), file.length(), file.toURI()
							.toURL().toString());
