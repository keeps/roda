...
<xsl:stylesheet version="2.0"
	...
	xmlns:prem="info:lc/xmlns/premis-v2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">
	...
	<xsl:variable name="agents2">
		<xsl:for-each select="$agents/span">
			<xsl:if
				test="((position()-1) &gt;= $fromAgent) and (($maxAgents=0) or (position() &lt;= ($fromAgent+$maxAgents)))">
				<xsl:copy-of select="." />
			</xsl:if>
		</xsl:for-each>
	</xsl:variable>
	...
	<xsl:variable name="events2">
		<xsl:for-each select="$events/span">
			<xsl:if
				test="((position()-1) &gt;= $fromEvent) and (($maxEvents=0) or (position() &lt;= ($fromEvent+$maxEvents)))">
				<xsl:copy-of select="." />
			</xsl:if>
		</xsl:for-each>
	</xsl:variable>
	...
	<xsl:variable name="files2">
		<xsl:for-each select="$files/span">
			<xsl:if
				test="((position()-1) &gt;= $fromFile) and (($maxFiles=0) or (position() &lt;= ($fromFile+$maxFiles)))">
				<xsl:copy-of select="." />
			</xsl:if>
		</xsl:for-each>
	</xsl:variable>
		...
		<xsl:if test="prem:relationship">
			<span class="section files">
				...
				<span class="pagination">
					<span class="fromFile">
						<xsl:value-of select="$fromFile" />
					</span>
					<span class="maxFiles">
						<xsl:value-of select="$maxFiles" />
					</span>
					<span class="totalFiles">
						<xsl:value-of select="count($files/span)" />
					</span>
				</span>
			</span>
			<span class="section events">
				...
				<span class="pagination">
					<span class="fromEvent">
						<xsl:value-of select="$fromEvent" />
					</span>
					<span class="maxEvents">
						<xsl:value-of select="$maxEvents" />
					</span>
					<span class="totalEvents">
						<xsl:value-of select="count($events/span)" />
					</span>
				</span>
			</span>
			<span class="section agents">
				...
				<span class="pagination">
					<span class="fromAgent">
						<xsl:value-of select="$fromAgent" />
					</span>
					<span class="maxAgents">
						<xsl:value-of select="$maxAgents" />
					</span>
					<span class="totalAgents">
						<xsl:value-of select="count($agents/span)" />
					</span>
				</span>
			</span>
		</xsl:if>
			...
			<span class="file">
				...
				<span class="header toggle-next">
					...
					<xsl:if test="prem:objectCharacteristics/prem:format">
						<xsl:if
							test="prem:objectCharacteristics/prem:format/prem:formatDesignation/prem:formatName/text() != ''">
