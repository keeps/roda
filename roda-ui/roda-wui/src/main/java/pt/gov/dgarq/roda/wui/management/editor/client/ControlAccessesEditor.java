...
public class ControlAccessesEditor implements MetadataElementEditor {
	...
	public ControlAccessesEditor() {
		...
		addControlAccess = new WUIButton(constants.controlAccessesAdd(),
	...
	public EadCValue getValue() {
		...
		for (ControlAccessEditor controlAccessEditor : controlAccesses) {
			ControlAccess controlaccess = controlAccessEditor
					.getControlAccess();
			if (controlaccess != null) {
				controlaccesses.add(controlaccess);
			}
		}
		return controlaccesses.size() == 0 ? null : new ControlAccesses(
				controlaccesses.toArray(new ControlAccess[] {}));
	}
	...
	public void setValue(EadCValue value) {
		if (value instanceof ControlAccesses) {
			...
			for (int i = 0; i < controlaccesses.getControlaccesses().length; i++) {
				ControlAccess controlaccess = controlaccesses
						.getControlaccesses()[i];
	...
	public static Widget getReadonlyWidget(EadCValue value) {
		...
		for (int i = 0; i < controlaccesses.getControlaccesses().length; i++) {
			...
			int row = 0;
			row = addToReadonlyWidget(constants.controlAccessesID(),
					controlaccess.getAttributeEncodinganalog(), sectionPanel,
					row);
			row = addToReadonlyWidget(constants.controlAccessesName(),
					controlaccess.getHead(), sectionPanel, row);
			row = addToReadonlyWidget(constants.controlAccessesDescription(),
					controlaccess.getSubject(), sectionPanel, row);
			row = addToReadonlyWidget(constants.controlAccessesLevel(),
					controlaccess.getFunction(), sectionPanel, row);
			row = addToReadonlyWidget(constants.controlAccessesReason(),
					controlaccess.getP(), sectionPanel, row);
			panel.add(sectionLabel);
			panel.add(sectionPanel);
			sectionLabel
					.addStyleName("wui-editor-controlaccesses-readonly-label");
			sectionPanel
					.addStyleName("wui-editor-controlaccesses-readonly-controlaccess");
		}
		panel.addStyleName("wui-editor-controlaccesses-readonly");
		return panel;
	}

	private static int addToReadonlyWidget(String label, String value,
	...
	public class ControlAccessEditor {
		...
		public ControlAccessEditor() {
			...
			sourceLabel
			.addStyleName("wui-editor-controlaccess-source-label");
			sourceBox.addStyleName("wui-editor-controlaccess-source-box");
			encodingLabel
					.addStyleName("wui-editor-controlaccess-encoding-label");
			...
			functionLabel
					.addStyleName("wui-editor-controlaccess-function-label");
		...
		public ControlAccess getControlAccess() {
			...
			if (!sourceBox.getText().isEmpty()
					&&!encodingBox.getText().isEmpty()
					&& !headBox.getText().isEmpty()
					&& !subjectBox.getText().isEmpty()
					&& !functionBox.getText().isEmpty()
					&& !pBox.getText().isEmpty()) {
				controlAccess = new ControlAccess(encodingBox.getText(),sourceBox.getText(),
						headBox.getText(), subjectBox.getText(),
						functionBox.getText(), pBox.getText());
			}
