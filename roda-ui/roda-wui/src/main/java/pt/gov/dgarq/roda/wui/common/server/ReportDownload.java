 ...
 * 
 */
public class ReportDownload extends javax.servlet.http.HttpServlet implements
	...
	public static class ReportInfo {
		 ...
		 * @param elements
		 */
		public ReportInfo(ContentAdapter adapter,
				ReportContentSource<Object> source, Object[] elements) {
	...
	protected String getTemplate(String bundleName, String templateName,
	...
	protected void doGet(HttpServletRequest request,
		...
		if (output.equals(REPORT_OUTPUT_PDF)) {
			...
			response.sendError(HttpServletResponse.SC_BAD_REQUEST,
					"Unknown requested output '" + output
							+ "'. Available outputs [" + REPORT_OUTPUT_PDF
							+ ", " + REPORT_OUTPUT_CSV + "]");
		}
	...
	private void getPDF(String type, Locale locale, String filename,
	...
	private void getCSV(String type, Locale locale, String filename,
			HttpServletRequest request, HttpServletResponse response)
			throws IOException {
	...
	protected String getContentAdapterFO(HttpServletRequest request,
		...
		reportInfo[7] = ContentAdapterHelper.translateSubList(
				adapter.getSublist(), total, locale);
		reportInfo[8] = ContentAdapterHelper.translateSorter(
				adapter.getSorter(), content, locale);

		logger.debug("Creating XML-FO");
		buffer.append(String.format(
				getTemplate(CONTENT_ADAPTER_BUNDLE_NAME, "xml.fo.header",
						locale), reportInfo));
		String dataFieldTemplate = getTemplate(CONTENT_ADAPTER_BUNDLE_NAME,
				"xml.fo.data.field", locale);
		String dataTemplate = getTemplate(CONTENT_ADAPTER_BUNDLE_NAME,
				"xml.fo.data", locale);
		for (Object element : elements) {
			String elementId = ServerTools.encodeXML(content
					.getElementId(element));
			Map<String, String> elementsFields = content.getElementFields(
					request, element);
				...
				if (entry.getValue() != null) {
					...
					if (ServerTools.isURL(entry.getValue())) {
						value = String.format("<fo:basic-link "
								...
								+ "</fo:basic-link>", entry.getValue());
					} else {
						value = ServerTools.encodeXML(entry.getValue());
					}
					fieldsFO.append(String.format(dataFieldTemplate,
							new Object[] { name, value }));
				}
			}
			buffer.append(String.format(dataTemplate, new Object[] { elementId,
					fieldsFO.toString() }));
		}

		buffer.append(getTemplate(CONTENT_ADAPTER_BUNDLE_NAME, "xml.fo.footer",
				locale));

		return buffer.toString();
	}

	protected String getReportFO(HttpServletRequest request, Locale locale)
			throws IOException {
		String ret = null;
		StringBuilder buffer = new StringBuilder();
		String header = getTemplate(REPORT_BUNDLE_NAME, "xml.fo.header", locale);
		String attributes = getTemplate(REPORT_BUNDLE_NAME,
				"xml.fo.attributes", locale);
		String attributesAttribute = getTemplate(REPORT_BUNDLE_NAME,
				"xml.fo.attributes.attribute", locale);
		String items = getTemplate(REPORT_BUNDLE_NAME, "xml.fo.items", locale);
		String itemsAttribute = getTemplate(REPORT_BUNDLE_NAME,
				"xml.fo.items.attribute", locale);
		String footerTemplate = getTemplate(REPORT_BUNDLE_NAME,
				"xml.fo.footer", locale);
	...
	private String getContentAdapterCSV(HttpServletRequest request,
			Locale locale) {
		...
		for (String header : headers) {
			...
			if (header.equals("id")) {
				...
				sBuffer.append(ServerTools.encodeCSV(content
						.getFieldNameTranslation(header)));
			}
		...
		for (Object element : elements) {
			...
			for (String header : headers) {
				...
				if (value != null) {
					sBuffer.append(ServerTools.encodeCSV(content
							.getFieldValueTranslation(value)));
				}
	...
	private String getReportCSV(HttpServletRequest request, Locale locale)
			throws ReportException, LoginException, RemoteException,
	...
	protected String getFilename(HttpServletRequest request, String type,
			String output) {
		String filename = "RODA_Report";
		if (type.equals(REPORT_TYPE_CONTENT_ADAPTER)) {
			ReportInfo info = getReportInfo(request.getSession());
			filename += "_"
					+ info.getSource().getReportTitle().replaceAll("\\s", "_");
		}
	...
	public synchronized void createPDFReport(HttpSession session,
			ReportContentSource contentSource, ContentAdapter adapter)
			throws PrintReportException {
		Object[] elements;
		try {
			elements = contentSource.getElements(session, adapter);
			session.setAttribute(REPORT_INFO_ATTRIB, new ReportInfo(adapter,
					contentSource, elements));
			notifyAll();
		} catch (Exception e) {
