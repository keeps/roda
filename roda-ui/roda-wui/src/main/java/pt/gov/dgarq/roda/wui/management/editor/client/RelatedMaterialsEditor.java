...
public class RelatedMaterialsEditor implements MetadataElementEditor {
	...
	public RelatedMaterialsEditor() {
		...
		addRelatedMaterial = new WUIButton(constants.relatedMaterialsAdd(),
		...
		relatedMaterialsLayout
				.addStyleName("wui-editor-relatedmaterials-layout");
	...
	public EadCValue getValue() {
		...
		for (RelatedMaterialEditor relatedMaterialEditor : relatedMaterials) {
			Relatedmaterial relatedmaterial = relatedMaterialEditor
					.getRelatedMaterial();
	...
	public void setValue(EadCValue value) {
		if (value instanceof Relatedmaterials) {
			...
			for (int i = 0; i < relatedmaterials.getRelatedmaterials().length; i++) {
				Relatedmaterial relatedmaterial = relatedmaterials
						.getRelatedmaterials()[i];
	...
	public static Widget getReadonlyWidget(EadCValue value) {
		...
		for (int i = 0; i < relatedmaterials.getRelatedmaterials().length; i++) {
			...
			if (p != null) {
				...
				addToReadonlyWidget(constants.relatedMaterialsDescription(),
						p.getText(), description, 0);
				panel.add(description);
				description
						.addStyleName("wui-editor-relatedmaterials-readonly-description");
			}
			if (archref != null) {
				Widget archrefWidget = ArchrefEditor.getReadonlyWidget(archref);
				panel.add(archrefWidget);
				archrefWidget
						.addStyleName("wui-editor-relatedmaterials-readonly-archref");
			}
		}
	...
	public class RelatedMaterialEditor {
		...
		public void setRelatedMateral(Relatedmaterial relatedmaterial) {
			if (relatedmaterial.getP() != null
					&& relatedmaterial.getP().getText() != null
		 ...
		 * @param listener
		 */
		public void removeRelatedMaterialListener(
				RelatedMaterialListener listener) {
			listeners.remove(listener);
		}
