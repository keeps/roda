...
public class ProcessInfoEditor implements MetadataElementEditor {
	...
	public void setValue(EadCValue value) {
		if (value instanceof ProcessInfo) {
			...
			for (int i = 0; i < processInfo.getpList().length; i++) {
				...
				if (p.getAttributeAltrender().equals(
						ATTRIBUTE_ALTRENDER_P_INPUT)) {
			...
			if (note != null && note.getP() != null
					&& note.getP().getText() != null) {
	...
	public static Widget getReadonlyWidget(EadCValue value) {
		...
		for (int i = 0; i < processInfo.getpList().length; i++) {
			...
			if (p.getAttributeAltrender().equals(ATTRIBUTE_ALTRENDER_P_INPUT)) {
				...
				inputPanel.add(ArchrefEditor.getReadonlyWidget(p.getArchrefs()
						.getArchrefs()[0]));
				inputLabel
						.addStyleName("wui-editor-processinfo-readonly-input-label");
			} else {
				...
				if (notes != null) {
					...
					if (note != null && note.getP() != null
							&& !note.getP().getText().isEmpty()) {
						FlexTable sectionPanel = new FlexTable();
						addToReadonlyWidget(
								constants.processInfoReasonForOutput(), note
										.getP().getText(), sectionPanel, 0);
						outputPanel.add(sectionPanel);
						sectionPanel
								.addStyleName("wui-editor-processinfo-readonly-output-note");
					}
				}

				outputPanel.add(ArchrefEditor.getReadonlyWidget(p.getArchrefs()
						.getArchrefs()[0]));
				outputLabel
						.addStyleName("wui-editor-processinfo-readonly-output-label");
			}
		}
		...
		if (note != null && note.getP() != null
				&& note.getP().getText() != null) {
			FlexTable sectionPanel = new FlexTable();
			addToReadonlyWidget(constants.processInfoReason(), note.getP()
					.getText(), sectionPanel, 0);
	...
	public class ProcessInfoEntryEditor {
		...
		public ProcessInfoEntryEditor(RedactionType type) {
			...
			redactionTypeLabel
					.addStyleName("wui-editor-processinfoentry-type-label");
			redactionTypePicker
					.addStyleName("wui-editor-processinfoentry-type-picker");
		...
		public P getProcessInfoEntry() {
			...
			if (redactionTypePicker.getSelectedIDType().equals(
					RedactionTypePicker.RedactionType.INPUT)) {
		 ...
		 * @param listener
		 */
		public void addProcessInfoEntryListener(
				ProcessInfoEntryListener listener) {
			listeners.add(listener);
		}
		 ...
		 * @param listener
		 */
		public void removeProcessInfoEntryListener(
				ProcessInfoEntryListener listener) {
			listeners.remove(listener);
		}
