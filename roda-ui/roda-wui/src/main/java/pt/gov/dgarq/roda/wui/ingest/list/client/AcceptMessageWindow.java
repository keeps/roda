...
public class AcceptMessageWindow extends WUIWindow {
  ...
  public AcceptMessageWindow(SIPState sip) {
    super(messages.acceptMessageWindowTitle(sip.getOriginalFilename()), 500, 250);
    this.sip = sip;
    ...
    accept = new WUIButton(constants.acceptMessageWindowAccept(), WUIButton.Left.ROUND, WUIButton.Right.ARROW_FORWARD);
    ...
    accept.addClickListener(new ClickListener() {
      ...
      public void onClick(Widget sender) {
        ...
        loading.show();
        IngestListService.Util.getInstance().acceptSIP(AcceptMessageWindow.this.sip.getId(), message.getText(),
    ...
    cancel = new WUIButton(constants.acceptMessageWindowCancel(), WUIButton.Left.ROUND, WUIButton.Right.CROSS);
  ...
  private void updateTemplates() {
    IngestListService.Util.getInstance().getAcceptMessageTemplates(commonConstants.locale(),
      new AsyncCallback<Map<String, String>>() {

        public void onFailure(Throwable caught) {
          logger.error("Error getting accept message templates", caught);
        }
