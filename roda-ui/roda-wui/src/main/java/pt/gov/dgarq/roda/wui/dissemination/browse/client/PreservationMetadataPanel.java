...
public class PreservationMetadataPanel extends Composite {
	...
	private static final String[] TIMELINE_ICONS = new String[] {
			"js/api/images/dark-green-circle.png",
			"js/api/images/dark-blue-circle.png",
			"js/api/images/dark-red-circle.png",
			"js/api/images/gray-circle.png" };

	private static final String[] TIMELINE_COLORS = new String[] { "green",
			"blue", "red", "gray" };
	...
	public void init() {
		if (!initialized) {
			...
			BrowserService.Util.getInstance().getPreservationsInfo(
					sdo.getPid(), new AsyncCallback<List<PreservationInfo>>() {
						...
						public void onSuccess(List<PreservationInfo> infos) {
							...
							for (PreservationInfo info : infos) {
								String icon = TIMELINE_ICONS[index
										% TIMELINE_ICONS.length];
								String color = TIMELINE_COLORS[index
								...
								if (info.isOriginal()) {
									repLabel.setText(messages
											.preservationRepOriginal(info
													.getLabel()));
								} else if (info.isNormalized()) {
									repLabel.setText(messages
											.preservationRepNormalized(info
													.getLabel()));
								} else {
									repLabel.setText(messages
											.preservationRepAlternative(info
													.getLabel()));
								}
								...
								repLayout
										.addStyleName("preservation-rep-layout");
			...
			representations
					.addStyleName("wui-metadata-preservation-header-reps");
	...
	private void initTimeline(List<PreservationInfo> infos, List<String> icons,
			List<String> colors) {
		loading.show();
		...
		BrowserService.Util.getInstance().getPreservationTimeline(rpoPIDs,
				icons, colors, commonConstants.locale(),
	...
	private class PreservationTimeLineRender implements ITimeLineRender {
		 ...
		 * @param pagerTimeUnit
		 */
		public PreservationTimeLineRender(TimelineInfo timeLineInfo,
				int primaryTimeUnit, int pagerTimeUnit) {
		...
		public void render(TimeLineWidget widget) {
			...
			for (Phase phase : timeLineInfo.getPhases()) {
				SpanHighlightDecoratorOptions phaseOpts = SpanHighlightDecoratorOptions
						.create();
				...
				SpanHighlightDecorator phaseDecorator = SpanHighlightDecorator
						.create(phaseOpts);
