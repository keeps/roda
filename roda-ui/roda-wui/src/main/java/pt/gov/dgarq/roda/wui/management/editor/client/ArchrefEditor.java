...
import pt.gov.dgarq.roda.core.data.eadc.Unitid;
import pt.gov.dgarq.roda.wui.common.client.images.CommonImageBundle;
import pt.gov.dgarq.roda.wui.common.client.widgets.IDTypePicker;
import pt.gov.dgarq.roda.wui.common.client.widgets.IDTypePicker.IDType;
...
public class ArchrefEditor {
	...
	public ArchrefEditor() {
		...
		addUnityId = new WUIButton(constants.archrefAdd(),
	...
	public Archref getValue() {
		...
		if (unitids.size() != 0) {
			...
			if (noteBox.isVisible() && !noteBox.getText().isEmpty()) {
				...
				archref.getNote().setAttributeAltrender(
						ATTRIBUTE_ALTRENDER_NOTE);
			}
		}
		return archref;
	}
	...
	public static Widget getReadonlyWidget(Archref value) {
		...
		if (archref.getUnitids() != null && archref.getUnitids().length > 0) {
			...
			for (int i = 0; i < archref.getUnitids().length; i++) {
				Unitid unitid = archref.getUnitids()[i];
				row = addToReadonlyWidget(IDTypePicker.getIDTypeLabel(unitid
						.getAttributeAltrender()), unitid.getText(),
						sectionPanel, row);
			}
			panel.add(label);
		...
		if (note != null && note.getP() != null
				&& note.getP().getText() != null) {
			FlexTable sectionPanel = new FlexTable();
			addToReadonlyWidget(constants.archrefNote(), note.getP().getText(),
					sectionPanel, 0);
			panel.add(sectionPanel);
	...
	public class UnitidEditor {
		...
		public void setUnitid(Unitid unitid) {
			if (unitid.getAttributeAltrender() != null
					&& unitid.getAttributeAltrender().equals(
							ATTRIBUTE_ALTRENDER_FULL_ID)) {
