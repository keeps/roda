...
public class ArrangementTableEditor implements MetadataElementEditor {
	...
	public ArrangementTableEditor() {
		...
		addRow = new WUIButton(constants.addRow(),
				WUIButton.Left.ROUND,
				WUIButton.Right.PLUS);
		addColumn = new WUIButton(constants.addColumn(),
				WUIButton.Left.ROUND,
				WUIButton.Right.PLUS);
		columns = 0;
	...
	private void updateLayout() {
		tableLayout.clear();
		for (int i = 0; i < columns; i++) {
			tableLayout.setWidget(0, i, ((ArrangementTableCellEditor) header
					.get(i)).getWidget());
		}
		...
		for (int i = 0; i < rows.size(); i++) {
			...
			for (int j = 0; j < columns; j++) {
				tableLayout.setWidget(i + 1, j,
						((ArrangementTableCellEditor) row.get(j)).getWidget());
			}
		...
		if (columns > 1) {
			for (int i = 0; i < columns; i++) {
				...
				columnRemove.addClickListener(new ClickListener() {
					...
					public void onClick(Widget sender) {
						if (Window
								.confirm(constants.removeColumnConfirmation())) {
							removeColumn(index);
							updateLayout();
						}
					}
				...
				tableLayout.getCellFormatter().setHorizontalAlignment(
						rows.size() + 1, i, HasAlignment.ALIGN_CENTER);
	...
	private ArrangementTableCellEditor createCellEditor(boolean header,
			String value) {
		ArrangementTableCellEditor cellEditor = new ArrangementTableCellEditor(
				header);
		if (value != null) {
		...
		if (value instanceof ArrangementTable) {
			...
			if (table.getArrangementTableGroups().length == 0) {
				...
				if (group.getHead().getRows().length > 1) {
					logger
							.warn("Arrangement table header as more than one row,"
				...
				for (int i = 0; i < group.getBody().getRows().length; i++) {
					...
					for (int j = 0; j < row.getEntries().length; j++) {
						rowValues.add(createCellEditor(false,
								row.getEntries()[j]));
					}
					rows.add(rowValues);
				}
			...
			if (table.getArrangementTableGroups().length > 1) {
				logger.warn("Arrangement table as more than one group,"
						+ " ignoring all but the first one");
			}
	...
	public EadCValue getValue() {
		...
		if (rows.size() == 0) {
			...
			ArrangementTableHead head = new ArrangementTableHead(
					new ArrangementTableRow[] { headRow });

			// body
			ArrangementTableRow[] bodyRows = new ArrangementTableRow[rows
					.size()];
			for (int i = 0; i < rows.size(); i++) {
			...
			ArrangementTableGroup group = new ArrangementTableGroup(columns,
					head, body);
