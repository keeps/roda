...
public class DescriptionElement implements SourcesChangeEvents {
	...
	private static EditorsIconBundle editorsIconBundle = (EditorsIconBundle) GWT
			.create(EditorsIconBundle.class);
	...
	public DescriptionElement(DescriptionObject object, String element,
			String description, EditMode[] editModes, boolean required,
			boolean valueAsHTML) {
		readonly = true;
		this.object = object;
	...
	private Widget createReadonlyWidget() {
		...
		if (value != null) {
			if (value instanceof Text) {
				...
				ret = new Label(Dissemination.getInstance()
						.getElementLevelTranslation((DescriptionLevel) value));
				...
				for (int i = 0; i < arrangementTable
						.getArrangementTableGroups().length; i++) {
					arrangementTables.add(new ArrangementTableGroupPanel(
							arrangementTable.getArrangementTableGroups()[i]));
				}
				ret = arrangementTables;
			} else if (value instanceof BioghistChronlist) {
				BioghistChronlist chronlist = (BioghistChronlist) value;
				BioghistChronitem[] chronitems = chronlist
				...
				for (int i = 0; i < languages.getLangmaterialLanguages().length; i++) {
					...
					Label language;
					try {
						language = new Label(constants.getString("lang_"
								+ langCode));
	...
	public Image createEditorModeIcon(EditMode mode) {
		Image ret = null;
		if (mode.equals(EditMode.TEXT_LINE) || mode.equals(EditMode.TEXT_AREA)
				|| mode.equals(EditMode.TEXT_BIGAREA)) {
			ret = editorsIconBundle.editorText().createImage();
		} else if (mode.equals(EditMode.DATE)) {
			ret = editorsIconBundle.editorDate().createImage();
		} else if (mode.equals(EditMode.CHRON_LIST)) {
			ret = editorsIconBundle.editorChronList().createImage();
		} else if (mode.equals(EditMode.LANGUAGES_LIST)) {
			ret = editorsIconBundle.editorLanguagesList().createImage();
		} else if (mode.equals(EditMode.ARRANGEMENT_TABLE)) {
			ret = editorsIconBundle.editorArrangementTable().createImage();
		} else {
			ret = editorsIconBundle.editorOther().createImage();
		}
		return ret;
	}
