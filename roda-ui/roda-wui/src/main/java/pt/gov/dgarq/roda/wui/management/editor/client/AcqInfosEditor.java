...
public class AcqInfosEditor implements MetadataElementEditor {
	...
	public AcqInfosEditor() {
		...
		addAcqInfo = new WUIButton(constants.acqInfosAdd(),
	...
	public EadCValue getValue() {
		...
		return acqinfos.size() == 0 ? null : new Acqinfos(
				acqinfos.toArray(new Acqinfo[] {}));
	}
	...
	public static Widget getReadonlyWidget(EadCValue value) {
		...
		for (int i = 0; i < acqinfos.getAcqinfos().length; i++) {
			...
			int row = 0;
			row = addToReadonlyWidget(constants.acqInfosText(), p.getText(),
					sectionPanel, row);
			row = addToReadonlyWidget(constants.acqInfosDate(), p.getDate(),
					sectionPanel, row);
			row = addToReadonlyWidget(constants.acqInfosNum(), p.getNum(),
					sectionPanel, row);
			row = addToReadonlyWidget(constants.acqInfosCorpname(),
					p.getCorpname(), sectionPanel, row);
	...
	public class AcqInfoEditor {
		...
		public Acqinfo getAcqInfo() {
			...
			if (!textBox.getText().isEmpty() || !dateBox.getText().isEmpty()
					|| !numBox.getText().isEmpty()
