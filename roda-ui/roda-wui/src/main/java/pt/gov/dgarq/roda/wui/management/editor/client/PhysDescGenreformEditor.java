...
public class PhysDescGenreformEditor implements MetadataElementEditor {
	...
	public EadCValue getValue() {
		return new PhysdescGenreform(sourceBox.getText(), idBox.getText(),
				nameBox.getText(), descriptionBox.getText());
	}
	...
	public boolean isEmpty() {
		return (!idBox.getText().equals("") && !sourceBox.getText().equals("")
				&& !nameBox.getText().equals("") && !descriptionBox.getText()
				.equals(""));
	}
	...
	public static Widget getReadonlyWidget(EadCValue value) {
		...
		PhysdescGenreform physdescGenreform = (PhysdescGenreform) value;
		int row = 0;
		row = addToReadonlyWidget(constants.physDescGenreformID(),
				physdescGenreform.getAttributeAuthfilenumber(), panel, row);
		row = addToReadonlyWidget(constants.physDescGenreformSource(),
				physdescGenreform.getAttributeSource(), panel, row);
		row = addToReadonlyWidget(constants.physDescGenreformName(),
				physdescGenreform.getAttributeNormal(), panel, row);
		row = addToReadonlyWidget(constants.physDescGenreformDescription(),
				physdescGenreform.getText(), panel, row);
