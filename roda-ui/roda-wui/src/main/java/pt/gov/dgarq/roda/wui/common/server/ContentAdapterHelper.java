...
public class ContentAdapterHelper {
	 ...
	 * @return
	 */
	public static String translateSubList(Sublist subList, int total,
			Locale locale) {
		...
		if (start == 0) {
			if (limit >= total) {
				...
				ret = String.format(messages.getString("subList.first"), limit,
						total);
			}
		} else {
			int end = (start + limit >= total) ? total : start + limit;
			ret = String.format(messages.getString("subList.interval"), start,
					end, total);
	 ...
	 * @return
	 */
	public static String translateSorter(Sorter sorter,
			ReportContentSource<Object> content, Locale locale) {
		ContentAdapterHelperMessages messages = new ContentAdapterHelperMessages(
				locale);
		String ret;
		...
		if (parameters.length > 0) {
			...
			for (SortParameter parameter : parameters) {
				...
				if (parameter.isDescending()) {
					ret += String.format(messages
							.getString("sorted.list.descending"), content
							.getFieldNameTranslation(parameter.getName()));
				} else {
					ret += String.format(messages
							.getString("sorted.list.ascending"), content
							.getFieldNameTranslation(parameter.getName()));
				}
			}
	 ...
	 * @return
	 */
	public static String translateFilter(Filter filter,
			ReportContentSource<Object> content, Locale locale) {
		ContentAdapterHelperMessages messages = new ContentAdapterHelperMessages(
				locale);
		String ret;
	...
	private static String translateFilterParameter(FilterParameter parameter,
			ReportContentSource<Object> content,
			ContentAdapterHelperMessages messages) {
		String ret;
		if (parameter instanceof SimpleFilterParameter) {
			SimpleFilterParameter simple = (SimpleFilterParameter) parameter;
			ret = String.format(messages.getString("filter.simple"), content
					.getFieldNameTranslation(simple.getName()), content
					.getFieldValueTranslation(simple.getValue()));
			...
			ret = String.format(messages.getString("filter.oneOfMany"), content
					.getFieldNameTranslation(oneOfMany.getName()), list);

		} else if (parameter instanceof RangeFilterParameter) {
			RangeFilterParameter range = (RangeFilterParameter) parameter;
			ret = String.format(messages.getString("filter.range"), content
					.getFieldNameTranslation(range.getName()), range
					.getFromValue(), range.getToValue());

		} else if (parameter instanceof RegexFilterParameter) {
			RegexFilterParameter regex = (RegexFilterParameter) parameter;
			ret = String
					.format(messages.getString("filter.regex"), content
							.getFieldNameTranslation(regex.getName()), regex
							.getRegex());

		} else {
			ret = "!!!NO TRANSLATION FOR FILTER TYPE "
