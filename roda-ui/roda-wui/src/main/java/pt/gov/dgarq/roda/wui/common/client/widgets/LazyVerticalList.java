...
public class LazyVerticalList<T> {
	...
	private static CommonMessages messages = (CommonMessages) GWT
	...
	public interface ContentSource<T> {
		 ...
		 * 
		 * @param adapter
		 *            Content adapter that defined the filter, sub-list and
		 *            sorting.
		 * @param callback
		 */
		...
		/**
		 * Get the total element count message to show on top of the lazy
		 * vertical list
		 * 
		 * @param total
	...
	public interface LazyVerticalListListener<T> {
		...
		/**
		 * On update begin, please use this to lock user options that could
		 * trigger another update of the lazy vertical list.
		 */
	 ...
	 * @param initialFilter
	 */
	public LazyVerticalList(ContentSource<T> contentSource, boolean autoUpdate,
			Filter initialFilter) {
		this(contentSource, DEFAULT_SCROLL_BLOCK_SIZE, DEFAULT_SCROLL_MAX_SIZE,
				DEFAULT_UPDATE_DELAY_MS, DEFAULT_MAX_REPORT_SIZE, initialFilter);
	 ...
	 * @param initialFilter
	 */
	public LazyVerticalList(ContentSource<T> contentSource,
			int autoUpdateDelayMs, Filter initialFilter) {
		this(contentSource, DEFAULT_SCROLL_BLOCK_SIZE, DEFAULT_SCROLL_MAX_SIZE,
				autoUpdateDelayMs, DEFAULT_MAX_REPORT_SIZE, initialFilter);
	}
	 ...
	 */
	public LazyVerticalList(ContentSource<T> contentSource,
			int scrollBlockSize, int scrollMaxSize, int updateDelayMs,
		...
		lazyScroll = new LazyScroll(listLayout, this.scrollBlockSize,
				this.scrollMaxSize, new Loader() {
					...
					public void load(final int offset, final int widgetOffset,
						...
						LazyVerticalList.this.contentSource.getElements(
								contentAdapter, new AsyncCallback<T[]>() {
					...
					public void remove(int offset, int widgetOffset, int limit,
						...
						last = last < listLayout.getWidgetCount() ? last
								: listLayout.getWidgetCount();
					...
					public void update(final int widgetOffset, int count,
							final AsyncCallback<Integer> updatedOffset) {
						contentAdapter.setSublist(new Sublist(widgetOffset,
								count));
						LazyVerticalList.this.contentSource.getElements(
								contentAdapter, new AsyncCallback<T[]>() {
									...
									public void onSuccess(T[] elements) {
										...
										updatedOffset.onSuccess(widgetOffset
												+ elements.length);
									}
		...
		printPDF.addClickListener(new ClickListener() {
			...
			public void onClick(Widget sender) {
				...
				if (total > LazyVerticalList.this.maxReportSize) {
					confirm = Window.confirm(messages.reportPrintMaxSize(
							LazyVerticalList.this.maxReportSize, total));
					printAdapter.setSublist(new Sublist(0,
							LazyVerticalList.this.maxReportSize));
				} else {
				...
				if (confirm) {
					printing = true;
					updateLoadingMessage();
					LazyVerticalList.this.contentSource.setReportInfo(
							printAdapter, constants.locale(),
							new AsyncCallback<Void>() {

								public void onFailure(Throwable caught) {
									logger.error("Error getting print id",
											caught);
								}
					...
					Window.open(GWT.getModuleBaseURL()
							+ "ReportDownload?type=CONTENT_ADAPTER&locale="
							+ constants.locale(), "_blank", "");
				}
		...
		printCSV.addClickListener(new ClickListener() {
			...
			public void onClick(Widget sender) {
				...
				if (confirm) {
					...
					Window.open(GWT.getModuleBaseURL()
							+ "ReportDownload?type=CONTENT_ADAPTER&locale="
							+ constants.locale() + "&output=CSV", "_blank", "");
				}
	...
	/**
	 * Add a user message to appear in loading bar, when no other loading
	 * messages are visible. To remove user message use
	...
	protected void updateLoadingMessage() {
		...
		loadingImage.setVisible(printing || scrollReseting || scrollUpdating
				|| scrollLoading || userMessage);
		loadingMessage.setVisible(printing || scrollReseting || scrollUpdating
				|| scrollLoading || userMessage);
	...
	protected void createWidgets(T[] elements, int offset, int widgetOffset) {
		for (int i = 0; i < elements.length; i++) {
			listLayout.insert(createWidget(elements[i]), offset - widgetOffset
					+ i);
		}
	}

	private Widget createWidget(final T element) {
		final ElementPanel<T> elementPanel = contentSource
				.getElementPanel(element);
		elementPanel.addClickListener(new ClickListener() {

			public void onClick(Widget sender) {
				if (selectedElementPanel != null
		...
		while (elementIndex >= 0 || elementPanelIndex >= 0) {
			if (elementIndex < 0) {
				...
				while (elementIndex >= 0) {
					ElementPanel<T> elementPanel = contentSource
							.getElementPanel(elements[elementIndex--]);
					listLayout.add(elementPanel);
				}
			} else {
				T updatedElement = elements[elementIndex];
				ElementPanel<T> elementPanel = (ElementPanel<T>) listLayout
						.getWidget(elementPanelIndex);
				...
				if (updatedElement.equals(localElement)) {
					...
					listLayout.insert(createWidget(updatedElement),
							elementPanelIndex + 1);
					elementIndex--;
					remainingElements.remove(updatedElement);
				} else {
	...
	protected void updateTotal() {
		contentSource.getCount(contentAdapter.getFilter(),
				new AsyncCallback<Integer>() {
					...
					public void onSuccess(Integer taskCount) {
						total = taskCount;
						totalLabel.setText(contentSource
								.getTotalMessage(taskCount));
					}
	 ...
	 * @param listener
	 */
	public void removeLazyVerticalListListener(
			LazyVerticalListListener<T> listener) {
