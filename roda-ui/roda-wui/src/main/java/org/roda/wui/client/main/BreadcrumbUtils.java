...
public class BreadcrumbUtils {
  ...
  public static List<BreadcrumbItem> getCreateRiskBreadCrumbs() {
    List<BreadcrumbItem> ret = new ArrayList<>();
    ...
    ret.add(new BreadcrumbItem(SafeHtmlUtils.fromSafeConstant(messages.newRiskTitle()), messages.newRiskTitle(),
      CreateRisk.RESOLVER.getHistoryPath()));
  ...
  public static List<BreadcrumbItem> getEditRiskBreadCrumbs(IndexedRisk risk) {
    List<BreadcrumbItem> ret = getRiskBreadCrumbs(risk);

    if (risk != null) {
      List<String> path = new ArrayList<>(EditRisk.RESOLVER.getHistoryPath());
      path.add(risk.getUUID());
      ret.add(
        new BreadcrumbItem(SafeHtmlUtils.fromSafeConstant(messages.editRiskTitle()), messages.editRiskTitle(), path));
    }
