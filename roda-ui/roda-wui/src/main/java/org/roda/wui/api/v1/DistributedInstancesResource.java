...
public class DistributedInstancesResource {
  ...
  @ApiResponses(value = {@ApiResponse(code = 200, message = "OK", response = StreamResponse.class),
          @ApiResponse(code = 404, message = "Not found", response = ApiResponseMessage.class)})
  public Response getUpdates( @ApiParam(value = "The instance identifier", required = true)  @PathParam(RodaConstants.API_PATH_PARAM_INSTANCE_IDENTIFIER) String instanceIdentifier) {
    // get user
    final User user = UserUtility.getApiUser(request);
    // delegate action to controller.
