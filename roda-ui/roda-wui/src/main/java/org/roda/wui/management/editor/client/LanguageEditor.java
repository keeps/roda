...
public class LanguageEditor implements MetadataElementEditor {
  ...
  public void setValue(EadCValue value) {
    if (value != null && value instanceof LangmaterialLanguages) {
      ...
      final String[] languages = langmaterialLanguages.getLangmaterialLanguages();
      ensureLoaded(new Command() {
        ...
        public void execute() {
          ...
          if (languagesFoundCount < languages.length) {
            logger.error("Some languages were not found" + " in language list: " + Tools.toString(languages));
          }
        }
  ...
  public EadCValue getValue() {
    ...
    return (langCodes.size() == 0) ? null : new LangmaterialLanguages((String[]) langCodes.toArray(new String[] {}));
  }
