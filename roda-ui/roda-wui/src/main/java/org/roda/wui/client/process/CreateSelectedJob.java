...
public abstract class CreateSelectedJob extends Composite {
  ...
  private final boolean isIngestJob;
  @UiField
  ...
  @UiField
  FlowPanel content;
  private CreateJobInformation jobInformationWidget;
  private CreateJobPluginSelection pluginSelectionWidget;
  private CreateJobOrchestration orchestrationWidget;

  protected CreateSelectedJob(final List<PluginType> pluginType, final boolean isIngest) {
    this.selected = LastSelectedItemsSingleton.getInstance().getSelectedItems();
    this.isIngestJob = isIngest;
    ...
    services
      .configurationsResource(
        s -> s.retrievePluginsInfo(pluginType, true, LocaleInfo.getCurrentLocale().getLocaleName()))
  ...
  public SelectedItems<? extends IsRODAObject> getSelected() {
    return selected;
  }

  private void init(PluginInfoList pluginInfoList) {
    navigationToolbar.withoutButtons().build();
    navigationToolbar.updateBreadcrumbPath(BreadcrumbUtils.getCreateJobBreadcrumbs(isIngestJob));
    ...
    jobInformationWidget = new CreateJobInformation(selected);
    pluginSelectionWidget = new CreateJobPluginSelection(pluginInfoList, selected.getSelectedClass(), !isIngestJob);
    ...
    pluginSelectionWidget.setOnCancelJobCallback(() -> {
      if (isIngestJob) {
        HistoryUtils.newHistory(IngestProcess.RESOLVER);
      } else {
        HistoryUtils.newHistory(ActionProcess.RESOLVER);
