 ...
 */
package org.roda.action.ingest.fastCharacterization.utils;
...
public class DroidUtils {
  ...
  public FileFormat execute(Path pathToIdentify) throws DroidException, IOException {
    ...
    try {
      ...
      if (results.getResults().size() < 1)
        ...
        throw new DroidException(String
          .format("More then one format detected, unknown format. Detected PRONOM PUIDs: %s", strBuilder.toString()));
      } else {
        // One format detected
