...
public class FullTextAction implements Plugin<AIP> {
  ...
  public Report execute(IndexService index, ModelService model, StorageService storage, List<AIP> list)
    ...
    // parser.parse(binary.getContent().createInputStream(), handler, metadata,
    // new ParseContext());
    // String content = handler.toString();
    // Path p = Files.createTempFile("tika", ".xml");
    // Files.write(p, content.getBytes());
    // Binary resource = (Binary) FSUtils.convertPathToResource(p.getParent(),
    // p);
    // model.createOtherMetadata(aip.getId(), file.getStoragePath().getName() +
    // ".xml", "tika", resource);
    // }
    // }
